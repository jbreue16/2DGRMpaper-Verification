    ...
    if use_CASEMA_reference:
        ...
        for target_zone in range(1, nRadialZones):
            ...
            for idx in range(len(settings)):
                ...
                old_name = str(output_path) + '/convergence_' + \
                    settings[idx]['name'] + '.json'
                new_name = str(output_path) + '/convergence_' + 'port' + str(
                    target_zone).zfill(3) + '_' + settings[idx]['name'] + '.json'
                rename_json_file(old_name, new_name)
        ...
        for idx in range(len(settings)):
            ...
            for target_zone in range(nRadialZones):
                ...
                file_name = str(output_path) + '/convergence_' + 'port' + str(
                    target_zone).zfill(3) + '_' + settings[idx]['name'] + '.json'
            ...
            L1EOC = np.insert(
                convergence.calculate_eoc(disc, L1Error), 0, 0.0)
            L2EOC = np.insert(
                convergence.calculate_eoc(disc, L2Error), 0, 0.0)
